...
df1 <- inner_join(o2_cb, ghg, by = c("kit_id", "transect_location"))
  


plot_grid(ggplot(df1 %>% filter(type == "dry") %>% 
                   filter(transect_location == "Sediment" | transect_location =="Wetland"), 
                 aes(delta_do_hr, pco2_uM)) + 
  geom_point() + geom_smooth() + ggtitle("Dry samples only"), 
  ggplot(df1 %>% filter(type == "wet") %>% 
           filter(transect_location == "Sediment" | transect_location =="Wetland"), 
         aes(delta_do_hr, pco2_uM)) + 
    geom_point() + geom_smooth() + ggtitle("Wet samples only"), 
  nrow = 1)



ggplot(ghg, aes(transect_location, pn2o_uM, fill = type)) + 
  geom_boxplot()

ggplot(df, aes(transect_location, d_pco2)) + 
  geom_boxplot() %>% 
...
ghg_no_outliers <- ghg_raw %>% 
  filter(pco2_uM < 7500) %>% #removes K015 Upland-dry (CO2 = 8641 uM)
  filter(pch4_c < 100) %>% #removes K21 Wetland-wet (CH4 = 143 nM)
